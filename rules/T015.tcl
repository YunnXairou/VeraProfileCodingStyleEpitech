    ...
    foreach token [getTokens $file 1 0 -1 -1 {ccomment cppcomment stringlit}] {
        ...
        if {[regexp -nocase $urlRe $tokenValue dummy link] == 1} {
            ...
            if {[string first "file:" $link] == 0} {
                report $rule $lineNumber "URL links to files are not allowed"
                continue
            }

            if {[regexp {[ \<\>\'\{\}\|\\\^\[\]]} $link] == 1} {
                report $rule $lineNumber "URL link contains illegal character(s)"
                continue
            }
            ...
            if {[string first "\#" $link [expr $pos + 1]] != -1} {
                report $rule $lineNumber "URL link contains invalid bookmark"
            }

            set completeLink [file join [file dirname $file] $plainLink]
            if {[file isfile $completeLink] == 0} {
                report $rule $lineNumber "URL points to non-existing file"
            }
        }
    }
}
